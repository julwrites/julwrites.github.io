...
I digress. Anyway, when I first heard about [OpenClaw](<OpenClaw website>), I was a little suspicious. I'm a software engineer by training, and it's been my life for 10 years. Now you want me to give root access to what? 
...
Naturally, I decided to host it on a spare [Raspberry Pi 4](<Raspberry Pi 4 website>) as an experiment. 
...
Installing on the Pi is easy; get the Raspberry Pi Imager, pick an image, flash it. I've had quite some success with High Endurance SD Cards (these Pi's really burn through SD card lifespans; most SD cards are made for high read, low write operations, not to be used as a low-cost SSD), so I went with this. 
...
It took a little bit of work to figure out why OpenClaw wasn't responding to my messages on Telegram (I set it up explictly to respond to my user ID). And then, I was in business. 
...
In the end, I pretty much re-wrote this entire article by hand, so maybe it wasn't super useful, but it was a pretty good fascimile of my writing, and a pretty good kick in the pants to myself. 
